...
uint32_t GetDllOffset(const char* dll, const int32_t offset) {
    ...
    if (!hMod)
        return NULL;
    if (offset < 0)
        return reinterpret_cast<uint32_t>(GetProcAddress(hMod, reinterpret_cast<LPCSTR>(-offset)));  // NOLINT(performance-no-int-to-ptr)
    return reinterpret_cast<uint32_t>(hMod) + offset;
}
...
bool setup_game_dlls() {
    ...
    DWORD pointerSaved[sizeof(pointersToLoad) / sizeof(PVOID)] = {};
    for (auto i = 0u; i < sizeof(pointersToLoad) / sizeof(PVOID); ++i)
        pointerSaved[i] = *static_cast<DWORD*>(pointersToLoad[i]);
    bool result = true;
    for (auto* ptr : pointersToLoad) {
        auto* p = static_cast<DWORD*>(ptr);
        const auto offset = GetDllOffset(*p);  // NOLINT(bugprone-narrowing-conversions, cppcoreguidelines-narrowing-conversions)
        if (!offset) { result = false; break; }
        *p = offset;
    }
    if (!result)
        for (auto i = 0u; i < sizeof(pointersToLoad) / sizeof(PVOID); ++i)
            *static_cast<DWORD*>(pointersToLoad[i]) = pointerSaved[i];
    return result;
}
...
Act* LoadAct(uint32_t actno, uint32_t seed, uint32_t difficulty, uint32_t TownLevelId) {
    return D2COMMON_LoadAct(actno, seed, TRUE, nullptr, difficulty, nullptr, TownLevelId, D2CLIENT_LoadAct_1, D2CLIENT_LoadAct_2);
}
